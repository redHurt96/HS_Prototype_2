...
namespace Cowsins.Player
{
    public class PlayerStats : MonoBehaviour, IDamageable
    {
        ...
        [Tooltip(" Color of healthStatesEffect on different actions such as getting hurted or healed"), SerializeField]
        ...
        [Tooltip("Minimum height ( in units ) the player has to fall from in order to take damage"), SerializeField, Min(1)]
        private float minimumHeightDifferenceToApplyDamage;

        [Tooltip("How the damage will increase on landing if the damage on fall is going to be applied"), SerializeField]
        ...
        public bool IsInBushes => _currentBush != null && InputManager.crouching;

        private BushesCharacterTrigger _currentBush;
        ...
        {
            if (playerMovement.canDash && playerMovement.dashing && playerMovement.damageProtectionWhileDashing) return;
        ...
        public void Heal(float healAmount_)
        {
            ...
            if (health + healAmount > maxHealth) // Check if heal exceeds health 
            {
                ...
                if (maxShield != 0)
                {
                    if (shield + remaining > maxShield)
                        shield = maxShield; // Then we have to apply the remaining heal to our shield 
        ...
        private void ManageFallDamage()
        {
            ...
            if (!playerMovement.grounded && transform.position.y > height || !playerMovement.grounded && height == null)
            ...
            if (playerMovement.grounded && height != null && transform.position.y < height)
            {
                ...
                if (heightDifference > minimumHeightDifferenceToApplyDamage)
                    Damage(heightDifference * fallDamageMultiplier);
        ...
        public void EnterBush(BushesCharacterTrigger bushesCharacterTrigger)
        {
            _currentBush = bushesCharacterTrigger;
        }

        public void EscapeBush(BushesCharacterTrigger bushesCharacterTrigger)
        {
            if (_currentBush == bushesCharacterTrigger)
                _currentBush = null;
        }
    }
}